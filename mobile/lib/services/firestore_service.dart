...
class FirestoreService {
  ...
  Future<void> setUserProfile(String uid, {required String email, String displayName = ''}) async {
  ...
  Future<void> updateUserFcmToken(String uid, String? token) async {
    await _firestore.collection(_users).doc(uid).update({'fcmToken': token});
  }
  ...
  Future<List<LimitRule>> getLimitRules(String uid) async {
    final snap = await _firestore.collection(_users).doc(uid).collection(_limitRules).get();
    return snap.docs.map((d) => LimitRule.fromMap(d.id, d.data())).where((r) => r.active).toList();
  }
  ...
  Future<List<UnlockRequest>> getPendingUnlockRequestsForPartner(String partnerUid) async {
    final snap = await _firestore
        ...
        .get();
    return snap.docs.map((d) => UnlockRequest.fromMap(d.id, d.data())).where((r) => !r.isExpired).toList();
  }
  ...
  Future<bool> consumeUnlockCode(String requestId, String code) async {
    ...
    await _firestore.collection(_unlockRequests).doc(requestId).update({'status': UnlockRequestStatus.used.value});
    return true;
  }

  Future<void> setBlockerConfig(String uid, BlockerConfig config) async {
    await _firestore.collection(_users).doc(uid).collection(_blockerConfig).doc('current').set(config.toMap());
  }

  Future<BlockerConfig?> getBlockerConfig(String uid) async {
    final doc = await _firestore.collection(_users).doc(uid).collection(_blockerConfig).doc('current').get();
    if (doc.data() == null) return null;
  ...
  Future<void> syncBlockerConfigFromRules(String uid, String displayName) async {
    ...
    final restrictedApps = rules.map((r) => r.appPackageOrCategory).where((p) => p.isNotEmpty).toSet().toList();
    ...
    await setBlockerConfig(uid, BlockerConfig(userId: uid, restrictedApps: restrictedApps, minutesAllowed: minutesAllowed, windowMinutes: windowMinutes, displayName: displayName));
  }

  Future<UsageState?> getUsageState(String uid) async {
    final doc = await _firestore.collection(_users).doc(uid).collection(_usageState).doc('current').get();
    if (doc.data() == null) return null;
    return UsageState.fromMap(uid, doc.data()!);
  }

  Future<void> setUsageState(String uid, UsageState state) async {
    await _firestore.collection(_users).doc(uid).collection(_usageState).doc('current').set(state.toMap());
  }

  Future<List<String>> getRestrictedAppPackages(String uid) async {
    final rules = await getLimitRules(uid);
    return rules.map((r) => r.appPackageOrCategory).where((p) => p.isNotEmpty).toSet().toList();
  }
}
